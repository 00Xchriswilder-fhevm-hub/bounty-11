...
This example demonstrates the FHE.sub operation to subtract two encrypted values. The subtraction is performed homomorphically without decrypting either operand. Note: No underflow protection in FHE - ensure the first operand is greater than or equal to the second in production.
