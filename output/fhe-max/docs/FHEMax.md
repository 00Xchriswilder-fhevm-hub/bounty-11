# FHE Maximum Operation Example

<!-- chapter: basic -->

## Overview

Demonstrates FHE.max operation to find the maximum of two encrypted values. This example demonstrates finding the maximum of two encrypted values, converting external encrypted inputs using Fully Homomorphic Encryption and shows how to manage FHE permissions for both contracts and users using external encrypted inputs with input proofs for verification.

## What You'll Learn

- **FHE.max operation** - How to perform this specific homomorphic operation on encrypted values
...
The `FHE.max()` function compares two encrypted values and returns the larger one, all without decrypting either value.
...
### Step 2: Perform FHE.max Operation

Call the function that performs `FHE.max` (e.g., `computeMax()`).
